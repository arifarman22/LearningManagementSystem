...
export default function CourseStudentsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { course, loading: courseLoading, forbidden: courseForbidden } = useCourse(id);
  const totalLessons = course?.lessons?.length ?? 0;
  const { students, loading, forbidden, error } = useCourseStudents(id, totalLessons);

  if (courseForbidden || forbidden) return <PermissionError message="You can only view students for your own courses." />;
    ...
    <div className="space-y-6">
      ...
      {loading || courseLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-neutral-300 bg-neutral-50 p-12 text-center">
          ...
          <p className="text-xs text-neutral-400 mt-1">Students will appear here once they enroll.</p>
        </div>
      ) : (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <ProgressBar
                        value={percentage}
                        size="sm"
                        color={percentage === 100 ? 'success' : 'brand'}
                        showValue
                      />
