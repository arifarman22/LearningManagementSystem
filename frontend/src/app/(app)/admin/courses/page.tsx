...
import { Plus, Search, Edit3, Trash2, Globe, EyeOff, BookOpen, Layers, Users } from 'lucide-react';
...
export default function AdminCoursesPage() {
  const { courses, loading, reload } = useAllCourses();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="All Courses"
        description={`${courses.length} total courses on the platform`}
        actions={
            ...
            {filtered.map((course) => (
              <TableRow key={course.documentId} className="hover:bg-neutral-50">
                ...
                <TableCell className="text-neutral-600">
                  {course.instructor?.username ?? <span className="text-neutral-300 italic">None</span>}
                </TableCell>
                <TableCell>
                  <span className="flex items-center gap-1 text-neutral-600">
                    <Layers size={13} /> {course.lessons?.length ?? 0}
                  </span>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button variant="ghost" size="xs" asChild>
                      <Link href={`/content/courses/${course.documentId}/edit`} className="no-underline" title="Edit">
                        <Edit3 size={14} />
                      </Link>
                    </Button>
                    <Button variant="ghost" size="xs" asChild>
                      <Link href={`/admin/courses/${course.documentId}/students`} className="no-underline" title="View student progress">
                        <Users size={14} />
                      </Link>
                    </Button>
                    <Button
