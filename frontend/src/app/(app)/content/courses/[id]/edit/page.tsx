...
export default function ContentEditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = React.use(params);
  const { course, loading, error } = useContentCourse(id);
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="rounded-2xl border border-neutral-200/60 bg-white p-6">
        <CourseForm initial={course} documentId={id} redirectBase="/content/courses" />
      </div>
    </div>
  );
}
