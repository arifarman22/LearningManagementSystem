...
import { useContentCourse } from '@/hooks/useContent';
...
export default function ContentCourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = React.use(params);
  const { course, loading, error, reload } = useContentCourse(id);
  ...
  async function togglePublish() {
    ...
    try {
      await api.patch(`/courses/${id}/${course.status === 'published' ? 'unpublish' : 'publish'}`, {});
      reload();
    } catch (e) {
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button variant="secondary" size="sm" loading={publishing} onClick={togglePublish}
            ...
            <Button variant="secondary" size="sm" asChild leftIcon={<Edit3 size={14} />}>
              <Link href={`/content/courses/${id}/edit`} className="no-underline">Edit</Link>
            </Button>
          </div>
        </div>
        {publishError && (
      ...
      <div className="flex gap-1 rounded-xl bg-neutral-100 p-1 w-fit">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${tab === t.id ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-neutral-700 whitespace-pre-wrap">{course.description || <span className="text-neutral-400 italic">No description.</span>}</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-xs font-medium text-neutral-500 uppercase tracking-wider mb-1">Instructor</p>
              <p className="text-sm text-neutral-700">{course.instructor?.username ?? 'None'}</p>
            </div>
            <div>
